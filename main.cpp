...
int main(int argc, char** argv){
	...
	if(argc==1){
		...
		system("del SB.exe");//Eliminamos el capturador de datos
		system("ren SB_bak.exe SB.exe 0>nul 1>nul 2>nul"); //Renombramos el ejecutable original (por si lo requiere otros programas)
		s2="SB.exe";
		for(int i=0;i<4;i++){
			f >> s1;
			s2+=" "+s1;
		...
		f.close();
		system(s2.c_str()); //Ejecutar SB.exe+SBconfig.txt
	}else{
